...
# SBS
cosmic_sbs_3.2 <- read.table(file.path(input_dir, "COSMIC_v3.2_SBS_GRCh37.txt"), sep="\t", head=T, stringsAsFactors = F)
rownames(cosmic_sbs_3.2) <- cosmic_sbs_3.2$Type
cosmic_sbs_3.2$Type <- NULL
cosmic_sbs_3.2 <- as.data.frame(t(as.matrix(cosmic_sbs_3.2)))
...
cosm_types <- list("v3.3_dbs" = cosmic_dbs_3.3, "v2_sbs" = signatures.cosmic, "v3.2_sbs" = cosmic_sbs_3.2, "v3.3_sbs" = cosmic_sbs_3.3)
...
## COSMIC V3.3 DNA repair only signatures

```{r COSMIC V3.3 DNA repair only sigs}
...
sigs <- as.matrix(sigs)
  ...
  heat_plot <- Heatmap(sigs, 
                       ...
                       rect_gp = gpar(col = "white"),
                       row_title = NULL, 
                       column_title = paste("COSMIC v3.3 DNA Repair Mutational Signature Weights"), 
                       column_title_side = "bottom")
heat_plot
...
stacked_plot <- ggplot(sigs_df_long, aes(fill=Signature, y=Exposure, x=Line)) + 
  ...
  scale_fill_brewer(palette="Set3") +
  xlab("Cell Line") +
  ylab("COSMIC SBS v3.3 Mutational Signature Weights") +
  theme_classic()
