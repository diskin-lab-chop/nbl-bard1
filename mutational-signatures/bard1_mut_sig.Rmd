...
## Run COSMIC V2 and V3.3 Signatures
Get [COSMIC V2 and V3 signatures](https://cancer.sanger.ac.uk/cosmic) for each sample.
This step will take some time. 

```{r run COSMIC V2 and V3 Signatures}
# determine the signatures contributing to each samples

# "v3.3_id" = cosmic_id_3.3
cosm_types <- list("v3.3_dbs" = cosmic_dbs_3.3, "v2_sbs" = cosmic_sbs_2, "v3.2_sbs" = cosmic_sbs_3.2, "v3.3_sbs" = cosmic_sbs_3.3)
  ...
  for (sig_type in names(cosm_types)) {
    ...
    else if (sig_type %in% c("v2_sbs", "v3.2_sbs", "v3.3_sbs")){
...
stacked_plot <- ggplot(sigs_df_long, aes(fill=Signature, y=Exposure, x=Line)) + 
  ...
  xlab("Cell Line") +
  ylab(paste("COSMIC ", sig_type, " Mutational Signature Weights")) +
  theme_classic()
...
## COSMIC V3.2 DNA repair only signatures

```{r COSMIC V3.2 DNA repair only sigs}
...
for (each in sample_ids) {
  ...
  sig_result = whichSignatures(tumor.ref = sigs_input, 
                         signatures.ref = cosm_types[["v3.2_sbs"]],
                         sample.id = each, 
                         contexts.needed = TRUE,
  ...
  cosmic_weights_mod <- cosmic_weights %>%
    arrange(Line) %>%
  select(Line, 1:ncol(cosmic_weights)-1) %>%
  write_tsv(paste(results_dir, "/v3.2_sbs-dnarepair-signature-exposures.tsv", sep = ""))
  print(cosmic_weights_mod)
...
sigs <- as.matrix(sigs)
  ...
  heat_plot <- Heatmap(sigs, 
                       ...
                       rect_gp = gpar(col = "white"),
                       row_title = NULL, 
                       column_title = paste("COSMIC v3.2 DNA Repair Mutational Signature Weights"), 
                       column_title_side = "bottom")
heat_plot
...
sigs_df_long$Signature <- factor(sigs_df_long$Signature, levels = c("SBS10b", "SBS26", "SBS36", "SBS6", "SBS3"))

stacked_plot <- ggplot(sigs_df_long, aes(fill=Signature, y=Exposure, x=Line)) + 
  geom_col(position = "fill") +
  scale_fill_brewer(palette="Set3") +
  xlab("Cell Line") +
  ylab("COSMIC SBS v3.2 DNA Repair Mutational Signature Weights") +
  theme_classic()
...
dev.set(dev.next())
# save heatmap 
  pdf(file.path(paste(plots_dir, "/cosmic_v3.2_dnarepair_sigs.pdf", sep = "")), width = 6, height = 4)
  print(heat_plot)
  print(stacked_plot)
  dev.off() 
```
...
##  Run MMSIG using COSMIC V2 and V3.2

```{r run mmsig}

mmsig_input <- maf %>%
  select(sample = Tumor_Sample_Barcode, chr = Chromosome, pos = Start_Position, ref = Reference_Allele, alt = Tumor_Seq_Allele2) %>%
  mutate(sample = case_when(sample == "IMR5-WT" ~ "Control",
                        sample == "IMR5-E287" ~ "E287fs",
                        sample == "IMR5-R112" ~ "R112*",
                        sample == "IMR5-R150" ~ "R150*",
                        TRUE ~ sample))

# COSMIC SBS versions of interest
v_of_interest <- list("v2" = cosmic_sbs_2_orig, "v3.2" = cosmic_sbs_3.2_orig)
version <- "v3.2"

# use all sigs or DNA repair only sigs
for (version in  names(v_of_interest)){
  mmsig_fit <- mm_fit_signatures(
  muts.input = mmsig_input,
  sig.input = v_of_interest[[version]],
  input.format = "vcf",
...
mmsig_weights <- mmsig_fit$estimate
mmsig_weights$sample <- rownames(mmsig_weights)
mmsig_weights <- mmsig_weights[,c(ncol(mmsig_weights), 1:ncol(mmsig_weights)-1)]

# remove underscore
names(mmsig_weights) <- gsub("_", " ", names(mmsig_weights), fixed = TRUE)
mmsig_weights <- mmsig_weights %>%
  arrange(sample) %>%
write_tsv(paste(results_dir, "/cosmic-", version, "-signature-exposures-mmsig.tsv", sep = ""))
...
sigs <- as.matrix(sigs)
  ...
  heat_plot <- Heatmap(sigs, 
                       ...
                       rect_gp = gpar(col = "white"),
                       row_title = NULL, 
                       column_title = paste("COSMIC ", version, " Mutational Signature Weights"), 
                       column_title_side = "bottom")
heat_plot
...
sigs_df_long <- sigs_df %>% pivot_longer(cols=sig_names,
                    names_to='Signature',
                    values_to='Exposure') #%>%
  # plot only DNA repair signatures
#  filter(Signature %in% dna_repair)

stacked_plot <- ggplot(sigs_df_long, aes(fill=Signature, y=Exposure, x=Line)) + 
  geom_col(position = "fill") +
  #scale_fill_brewer(palette="Set3") +
  xlab("Cell Line") +
  ylab(paste("COSMIC ", version, " Mutational Signature Weights")) +
  theme_classic()
...
dev.set(dev.next())
# save heatmap 
  pdf(file.path(paste(plots_dir, "/cosmic_", version, "_sigs-mmsig.pdf", sep = "")), width = 6, height = 4)
  print(heat_plot)
  print(stacked_plot)
  dev.off()
