...
cosm_types <- list("v2_sbs" = signatures.cosmic, "v3.3_sbs" = cosmic_sbs_3.3, "v3.3_dbs" = cosmic_dbs_3.3, "v3.3_id" = cosmic_id_3.3)

  for (sig_type in cosm_types) {
    cosmic_weights = data.frame()
    for (each in sample_ids) {
  
  sig_result = whichSignatures(tumor.ref = sigs_input, 
                         signatures.ref = sig_type,
                         sample.id = each, 
  ...
  mutate(Line = case_when(Line == "IMR5-WT" ~ "Control",
                        ...
                        TRUE ~ Line)) %>%
    arrange(Line) %>%
  select(Line, 1:ncol(cosmic_weights_mod)-1) %>%
  write_tsv(paste(results_dir, "/", names(cosm_types)[i], "-signature-exposures.tsv", sep = ""))
...
sigs <- as.matrix(sigs)
  ...
  heat_plot <- Heatmap(sigs, 
                       ...
                       row_title = NULL, 
                       column_title = "COSMIC Mutational Signature Weights", 
                       column_title_side = "bottom")
heat_plot

dev.set(dev.next())
# save heatmap 
  pdf(file.path(plots_dir, "bard1_line_cosmic_sigs.pdf"), width = 6, height = 4)
  print(heat_plot)
  dev.off()
